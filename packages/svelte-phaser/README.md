...
From a workspace / npm (components + core):

```sh
npm install svelte-phaser phaser svelte
```
...
- **JSR**: `npx jsr publish` from this directory publishes the core layer as
  `@easierbycode/svelte-phaser` (see `jsr.json`; validate with
  `npx jsr publish --dry-run`).
- **npm**: the full package (components + core) can be published as-is — it
  ships uncompiled `.svelte`/TS source, which `@sveltejs/vite-plugin-svelte`
  compiles in the consuming app (the `svelte` export condition points at the
  source).
